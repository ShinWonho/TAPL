...
def substitute(t1: term, x: String, t2: term): term = t1 match {
  ...
  case AppStep1(f, t3) => AppStep1(substitute(f, x, t2), substitute(t3, x, t2))
  case AppStep2(f, t3) => AppStep2(substitute(f, x, t2), substitute(t3, x, t2))
  case _ => t1
}
...
def small_step(t:term):term = t match {
  ...
  case AppStep1(f, t) => t match {
    case Fun(_, _) => AppStep2(f, t)
    case Var(_) => AppStep2(f, t)
    case Wrong => t
    case _ => AppStep1(f, small_step(t))
  }
  case AppStep2(f, t) => f match {
...
def Four(n:Int):term = big_step(App(Succ(n), Two(n)))
...
big_step(Three(0))
//LCThree()
big_step(App(App(Plus(0), One(0)), One(0)))
//LCFour()
